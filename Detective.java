	...
	{ Scanner input=new Scanner(System.in);
	  Random rand = new Random();
			...
			while(no_of_m>0 && ((n-no_of_m)>no_of_m))
			{
				...
				int d=0;
			if(Game_Mafia.arr[Game_Mafia.m_no]!=null)				//If I am alive
			{	System.out.println("Choose a Player to test:");
				int c=0; 
				 while(c==0)
				 {	try { input=new Scanner(System.in); d=input.nextInt(); c=1;}
					catch(InputMismatchException ex) { 
						System.out.println("Enter a number:");
					} }
				while((d>Game_Mafia.m_no && d<=(Game_Mafia.m_no+Game_Mafia.d_no)) || Game_Mafia.arr[d-1]==null)
				{
					System.out.println("You cannot test a detective or a dead player. Choose a player to test:");
					d=input.nextInt();
				}
			}
			
			else //if(no_of_d>0)				//If I have died but detectives are there
			{
				...
				if((n-no_of_m)<=no_of_m)
					break;
				
				if(no_of_d>0 && d>0 && d<=Game_Mafia.m_no)				//if detectives alive and chose mafia
				{
					System.out.println("Player"+d+" was a mafia.");
				...
				{
					System.out.println("Player"+d+" was not a mafia.");
