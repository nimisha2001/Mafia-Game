...
class Mafia extends Player {
	...
	Random rand = new Random();
		...
		while(no_of_m>0 && ((n-no_of_m)>no_of_m))
		{
			...
			int t=0;
			if(Game_Mafia.arr[0]!=null)
			{System.out.println("Choose a target:");
			int c=0; 
			while(c==0)
			{try { input=new Scanner(System.in); t=input.nextInt(); c=1;}
			catch(InputMismatchException ex) { 
				System.out.println("Enter a number:");
			} }
			while((t>0 && t<=Game_Mafia.m_no) || Game_Mafia.arr[t-1]==null)
			{
				System.out.println("You cannot target a mafia or a dead player. Choose a player to target:");
				t=input.nextInt();
			} }
			...
			{
				if(Game_Mafia.arr[0]!=null)				//If i am alive
