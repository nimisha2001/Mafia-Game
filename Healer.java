...
class Healer extends Player {
	...
	Random rand = new Random();
			...
			int c=0; while(c==0)
			{	try { input=new Scanner(System.in); h=input.nextInt(); c=1;}
				catch(InputMismatchException ex) { 
					System.out.println("Enter a number:");
				} }
				while(h>(Game_Mafia.m_no+Game_Mafia.d_no) && h<=(Game_Mafia.N-Game_Mafia.c_no) || Game_Mafia.arr[h-1]==null)
					{ System.out.println("You can't choose a healer or a dead player. Choose someone else:");
					h=input.nextInt();}
			}
			...
			else
			{
				...
				{
					if(Game_Mafia.arr[(Game_Mafia.m_no+Game_Mafia.d_no)]!=null)				//If i am alive
